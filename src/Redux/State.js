...
export let addPost = () => {
    ...
    if (state.profilePage.newPostText != "") {
        ...
        state.profilePage.newPostText = "Новый пост";
        rerender();
    }
    else {alert ("Пост должен содержать контент !")}

};

export let sendMassage = () => {
    let newMassage = {
    id: 5,
...
export let postTextChange = (enteredPostText) => {
    state.profilePage.newPostText = enteredPostText
    rerender(state);
}

export let massageTextChange = (enteredMassageText) => {
    state.dialogsPage.newMassageText = enteredMassageText;
    rerender(state);
}

export let bonkRerender = (observer) => {
    rerender = observer;
}
