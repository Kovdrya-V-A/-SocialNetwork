...
const Nav = () => {
    return (
            <nav className={s.nav}>
                ...
                <div className={s.navItem}>
                    <button className={s.logoutButton} onClick={() => {window.location.reload(); localStorage.removeItem("userToken")}}><NavLink className={`${s.link} ${s.logoutLink}`} to ="/">Log out</NavLink></button>
                </div>
            </nav>
    )
}
